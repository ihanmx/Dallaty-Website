...
import { Box, Typography, FormControl, InputLabel, Select, MenuItem, Stack, Button } from "@mui/material";
...
const DatabaseViewer = () => {
    ...
    const tables = ['payments', 'lostreports', 'foundreports'];
...
//When deletion is confirmed call API to delete rows
    const handleConfirmDelete=async()=>{
        setLoading(true);
        const idFieldMap={'lostreports':'reportid',
...
//set loading false and close dialog
    ...
    return (
          <Box sx={{ p: 4, height: '90vh', display: 'flex', flexDirection: 'column' }}>
    ...
    setSelectedRows(ids || []); //converts from set to array
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete {selectedRows.length} selected row(s) from the <strong>{currentTable}</strong> table?
                        <br /><br />
